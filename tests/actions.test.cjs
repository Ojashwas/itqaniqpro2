...
test('visible role selection filters assigned accounts and enters the correct departmental workspace',()=>{
  ...
  try{
    ...
    d.querySelector('#sign-out').click();selectDemoAccount(w,'lead-2');assert.match(d.querySelector('#login-role-description').textContent,/Review department/);d.querySelector('#login-form').dispatchEvent(new w.Event('submit',{cancelable:true}));assert.equal(s.currentUser().role,'lead');assert.equal(s.currentUser().department,'Traffic & Licensing');
  }finally{dom.window.close()}
});
