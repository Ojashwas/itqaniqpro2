...
Interactive CONFEDINTIAL CLIENT performance-management prototype based on the supplied functional specification. All people, measures, results and insights are synthetic examples, not official client records. The ITQAN IQ mark in the sidebar is the product brand; the client seal beside it is a placeholder, not the official client emblem.
...
Open http://localhost:5180. Run `npm run check` for syntax and `npm test` for the calculation suite and the ten-view smoke test.

## Suggested walkthrough

1. Review the executive dashboard and strategic goal achievement.
2. Open Strategy & objectives and drill into a KPI.
3. Open Service completion time to review the critical breach and linked recovery action.
4. Record a new actual. Status, achievement and equal-weight goal scores update automatically; changes persist locally.
5. Change the reporting period. Every view — scores, trends, breaches, notifications, exports and the copilot — re-reads the selected month, and closed periods become read-only.
6. Open Action & recovery and start an action. Closing requires the current and previous period to be Green.
7. Register a KPI in the registry. The new definition stays in Draft until approved from its detail card by the simulated PMO user, then carries no historical observations until an actual is recorded.
8. Try Ask ITQAN's risk, service and forecast prompts.
9. Export a CSV or use the report print preview to save a PDF.
10. Switch to Arabic to preview RTL navigation and the executive dashboard.

## Implemented prototype scope

Ten navigation views cover strategic alignment, registry, data entry, deterministic scoring, illustrative insights, impact and escalation, recovery actions, reports, preferences and activity history. Eight seeded KPIs span four strategic goals and departments. Search, filters, notifications, forms, local persistence, CSV export and print styling work without backend services.
