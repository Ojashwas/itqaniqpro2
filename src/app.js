...
let page='dashboard',ar=false,periodIndex=Data.latest,reportingPeriodIndex=Data.latest,department='All departments',filter='All statuses',query='';
...
function persist(event,eventDepartment){
  if(event)audit.unshift({event,actor:currentUser()?.name||'Demo user',actorId:currentUser()?.id,department:eventDepartment||(globalAccess()?(department==='All departments'?null:department):currentUser()?.department),time:new Date().toLocaleString('en-GB')});
...
function decideRequest(id,decision,comment){
  ...
  const record=requestEntity(r);if(decision==='Approve'&&(!record||entityVersion(record)!==r.baseVersion))throw Error('This record has changed. Reject this request and submit a fresh version.');
  if(decision==='Approve'&&r.status==='Strategy review'){
    if(r.type==='KPI publication'){Data.validateDefinition(record,goals);record.draft=false;record.createdIndex=Data.latest;}
      ...
      const candidate={...record,...r.proposed};if(planGaps(candidate).length||candidate.milestones.some(m=>!m.owner||!m.due||m.due>candidate.due))throw Error('The corrective plan and milestone assignments must be complete.');
...
function chart({values,labels,target,suffix='',caption=''}){
  ...
  return `<div class="chart"><svg viewBox="0 0 ${W} ${H}" role="img" aria-label="${escapeHtml(caption||'Trend chart')}"><defs><linearGradient id="${id}" x1="0" y1="0" x2="0" y2="1"><stop stop-color="#2ba97d" stop-opacity=".22"/><stop offset="1" stop-color="#2ba97d" stop-opacity="0"/></linearGradient></defs>${axis}${targetLine}${runs.filter(r=>r.length>1).map(r=>`<polygon points="${r[0][0].toFixed(1)},${base} ${path(r)} ${r[r.length-1][0].toFixed(1)},${base}" fill="url(#${id})"/>`).join('')}${runs.filter(r=>r.length>1).map(r=>`<polyline points="${path(r)}" fill="none" stroke="#1a8f68" stroke-width="2.5" stroke-linejoin="round" stroke-linecap="round"/>`).join('')}${runs.flat().map(p=>`<circle cx="${p[0].toFixed(1)}" cy="${p[1].toFixed(1)}" r="4" fill="#fff" stroke="#1a8f68" stroke-width="2.5"/>`).join('')}${targetLabel}${labels.map((l,i)=>`<text class="axis" x="${x(i).toFixed(1)}" y="${H-10}" text-anchor="middle">${l}</text>`).join('')}</svg></div>`;
}
...
function render(){
  ...
  $('#app').innerHTML=`<aside class="sidebar"><div class="brand">${brandMark(46)}<div><strong>ITQAN <span>IQ</span></strong><small>INTELLIGENCE IN PERFORMANCE</small></div></div><div class="org"><span class="seal">♜</span><div><strong>${escapeHtml(appConfig.orgName)}</strong><small>${t('United Arab Emirates','الإمارات العربية المتحدة')}</small></div></div><nav>${nav.filter(n=>n[0]!=='admin'||currentUser()?.role==='admin').map(([id,en,a],i)=>`${i===0?`<div class="nav-label">${t('WORKSPACE','مساحة العمل')}</div>`:i===7?`<div class="nav-label">${t('GOVERNANCE','الحوكمة')}</div>`:''}<button class="nav-item ${page===id?'active':''}" data-page="${id}"${page===id?' aria-current="page"':''}><span class="nav-icon" aria-hidden="true">${icons[id]}</span>${t(en,a)}${id==='approvals'&&visibleApprovals().some(canDecide)?`<span class="count">${visibleApprovals().filter(canDecide).length}</span>`:id==='actions'&&openActions?`<span class="count">${openActions}</span>`:id==='ai'?'<span class="count">IQ</span>':''}</button>`).join('')}</nav><div class="side-bottom"><div class="help-card"><strong>✧ ${t('Clarity. Confidence. Impact.','وضوح. ثقة. أثر.')}</strong><p>${t('Turn performance intelligence into meaningful action.','حوّل ذكاء الأداء إلى إجراءات مؤثرة.')}</p><button data-page="ai">${t('Meet your AI copilot','تعرف على مساعدك الذكي')} ↗</button></div><div class="user"><span class="avatar">${escapeHtml(currentUser()?.name.split(' ').map(v=>v[0]).slice(0,2).join('')||'IQ')}</span><div><strong>${escapeHtml(currentUser().name)}</strong><small>${escapeHtml(roleNames[currentUser().role])} · ${escapeHtml(currentUser().department||'All departments')}</small></div></div></div></aside><main class="main"><header class="topbar"><div class="breadcrumb">${t('Workspace','مساحة العمل')} / <b>${t(nav.find(n=>n[0]===page)[1],nav.find(n=>n[0]===page)[2])}</b></div><div class="top-actions"><label class="search-box"><span aria-hidden="true">⌕</span><input class="search" id="global-search" placeholder="${t('Search ITQAN IQ…','البحث في المؤشرات…')}" aria-label="${t('Search KPIs','البحث في المؤشرات')}"></label><span class="demo">${escapeHtml(roleNames[currentUser().role])}</span>${canWrite()?`<button class="primary" id="create-menu">＋ ${t('Create','إنشاء')}</button>`:''}${currentUser()?.role==='admin'?`<button class="ghost" data-page="admin" id="admin-shortcut">⚙ ${t('Administration','الإدارة')}</button>`:''}<button class="ghost" id="language">${ar?'English':'العربية'}</button><button class="icon-button" id="notifications" aria-label="${t('Notifications','التنبيهات')}">♧${attention()?`<span class="dot" aria-hidden="true"></span>`:''}</button><span class="avatar">${escapeHtml(currentUser()?.name.split(' ').map(v=>v[0]).slice(0,2).join('')||'IQ')}</span><button class="icon-button" id="sign-out" title="${t('Sign out','تسجيل الخروج')}" aria-label="${t('Sign out','تسجيل الخروج')}">⇥</button></div></header><div class="content">${heading()}${body()}<footer class="footer"><span>ITQAN IQ · ${t('A clear line of sight. A greater impact.','رؤية واضحة. أثر أكبر.')}</span><span>${t('Interactive prototype · Synthetic data','نموذج تفاعلي · بيانات اصطناعية')} · ${period()}</span></footer></div></main>`;
  bind();
}
...
function renderLogin(){
  ...
  $('#login-language').onclick=()=>{ar=!ar;render()};
...
function dataHub(){
  const items=scoped().filter(k=>!dataSourceFilter||k.source===dataSourceFilter),sources=[...new Set(items.map(k=>k.source))];
...
function aiView(){
  ...
  return `<section class="panel assistant-home"><div class="eyebrow">✧ ${t('YOUR WORKSPACE GUIDE','دليلك في التطبيق')}</div><h2>${t('Find what you need. Know what to do next.','اعثر على ما تحتاجه واعرف خطوتك التالية.')}</h2><form id="ask-form"><label class="sr-only" for="ask-input">${t('Search ITQAN IQ or ask for help','اسأل أو ابحث عن مؤشر')}</label><input id="ask-input" required maxlength="240" placeholder="${t('Search goals, KPIs, plans, approvals, sources or ask how…','جرّب «إعداد المستهدفات» أو اسم مؤشر…')}"><button class="primary">${t('Find','بحث')} →</button></form><div class="assistant-prompts"><button data-ask="help">How does ITQAN IQ work?</button><button data-ask="approvals">Find approvals</button><button data-ask="attention">${t('What needs attention?','ما الذي يحتاج انتباهاً؟')}</button><button data-ask="targets">${t('How do I set targets?','كيف أعد المستهدفات؟')}</button><button data-ask="data">${t('Where do actuals come from?','من أين تأتي القيم الفعلية؟')}</button></div><p class="assistant-note">${t('Demo guide · Based on workspace data, not a connected AI service.','دليل تجريبي · يعتمد على بيانات التطبيق وليس خدمة ذكاء اصطناعي متصلة.')}</p></section>
  <div class="assistant-summary"><button data-ask="attention"><strong>${attention}</strong><span>${t('KPIs need attention','مؤشرات تحتاج انتباهاً')} →</span></button><button data-ask="recovery"><strong>${active.length}</strong><span>${t('Active plans','إجراءات تعافٍ نشطة')} →</span></button><button data-ask="overdue"><strong>${active.filter(overdue).length}</strong><span>${t('Overdue actions','إجراءات متأخرة')} →</span></button></div>
  <section class="panel assistant-answer" id="chat" aria-live="polite" aria-atomic="true">${assistantAnswer('attention')}</section>
  <section class="assistant-navigation"><h2>${t('Go directly to…','انتقل مباشرة إلى…')}</h2><div class="assistant-shortcuts">${[['strategy','◇',t('Objectives & connections','الأهداف والروابط'),t('See how KPIs and recovery support your goals.','شاهد ارتباط المؤشرات والتعافي بأهدافك.')],['targets','▤',t('Set KPI targets','إعداد مستهدفات المؤشرات'),t('Choose a KPI and edit its target.','اختر مؤشراً وعدّل مستهدفه.')],['data','▥',t('View incoming actuals','عرض القيم الفعلية الواردة'),t('Review values from source applications.','راجع قيم التطبيقات المصدرية.')],['actions','☑',t('Manage recovery','إدارة التعافي'),t('Follow up on owners, deadlines and evidence.','تابع المسؤولين والمواعيد والأدلة.')]].map(([id,icon,label,desc])=>`<button data-assistant-nav="${id}"><span class="assistant-shortcut-icon" aria-hidden="true">${icon}</span><span><strong>${label}</strong><small>${desc}</small></span><span aria-hidden="true">→</span></button>`).join('')}</div></section>`;
}
...
const recoveryCount=()=>appConfig.recoveryPeriods===2?'two':String(appConfig.recoveryPeriods);
...
function planGaps(a){
  ...
  const p=a.plan||{};
  return ['rootCause','correction','successCriteria','reviewer','cadence','nextReview'].filter(key=>!p[key]?.trim()).concat(!validDate(p.nextReview)||!['Weekly','Fortnightly','Monthly'].includes(p.cadence)?['review schedule']:[]).concat(!a.milestones.length?['milestones']:[]);
}
...
function recordMonitoring(a,{outcome,evidence,blockers,nextReview}){
  ...
  if(outcome==='Effective'&&((a.planType!=='Action plan'&&!actionReady(a))||a.milestones.some(m=>!m.done||!m.evidence?.trim()||!m.owner?.trim()||!m.due||m.due>a.due)))throw Error(t(`Effectiveness needs completed milestones and ${recoveryCount()} Green KPI results.`,'تتطلب الفعالية مراحل مكتملة ونتيجتين خضراوين للمؤشر.'));
...
function validateTransition(a,next,evidence=''){
  ...
  if(next==='In progress'&&a.status==='Open'){if(planGaps(a).length||a.milestones.some(m=>!m.owner||!m.due||m.due>a.due))throw Error(t('Complete the corrective plan and assign dated milestones before starting.','أكمل الخطة وحدد مسؤولاً وتاريخاً للمراحل قبل البدء.'));a.status=next;a.startedAt=new Date().toISOString()}
  else if(next==='Closed'&&a.status==='In progress'){
    if(a.planType!=='Action plan'&&!actionReady(a))throw Error(t(`Closure requires ${recoveryCount()} consecutive Green periods for a published KPI.`,'يتطلب الإغلاق فترتين خضراوين متتاليتين لمؤشر منشور.'));
...
function actionDetail(id,workingCopy){
  ...
  if($('#submit-plan'))$('#submit-plan').onclick=()=>{try{if(planGaps(a).length||a.milestones.some(m=>!m.owner||!m.due||m.due>a.due))throw Error('Complete the plan and assign milestone owners and dates.');submitRequest('Plan approval','Plan',stored,a,a.title);render();actionDetail(a.id)}catch(err){$('#submit-plan-error').textContent=err.message}};
...
function drawer(title,html){
  ...
  $('#close-modal').focus();
  bindKpis();
  bindConnections();bindGovernance();
}
...
function register(editId){
  ...
  $('#register-form').onsubmit=e=>{
    ...
    const draft={
      ...
      target:Number(read('target')),amber:Number(read('amber')),baseline:0,
    ...
    try{assertWrite(draft);validateLookup('kpiTypes',draft.type,editing?.type);validateLookup('units',draft.unit,editing?.unit);Data.validateDefinition(draft,goals)}catch(err){return toast(err.message)}
